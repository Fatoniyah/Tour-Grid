...
class MdBookingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $md_booking_id = $booking->md_booking_id ;
        $md_tour_id    = $md_tour->md_tour_id;

        // dd($booking);

        return redirect()->route('success', ['md_tour_id' => $md_tour_id, 'md_booking_id' => $md_booking_id])
            ->with('success', 'การจองของคุณสำเร็จ');
    }

    public function success($md_booking_id,$md_tour_id)
    {
        $booking = MdBooking::find($md_booking_id);
        // dd($booking);

        $md_tour = MdTour::find($md_tour_id);

        return view('success', compact('booking','md_tour'));
    }
